...
export const FinanceProvider = ({ children }) => {
  ...
  const createBudget = async (budgetData) => {
    ...
    try {
      ...
      if (useMockData) {
        ...
        // Update dashboard summary with new budget
        if (dashboardSummary && dashboardSummary.budget_vs_spending) {
          const updatedBudgetVsSpending = [...dashboardSummary.budget_vs_spending];
          const existingIndex = updatedBudgetVsSpending.findIndex(
            item => item.category === budgetData.category
          );
          
          if (existingIndex >= 0) {
            updatedBudgetVsSpending[existingIndex].budget = budgetData.amount;
          } else {
            updatedBudgetVsSpending.push({
          ...
          setDashboardSummary({
            ...dashboardSummary,
            budget_vs_spending: updatedBudgetVsSpending
          });
        }
  ...
  const updateSavingsGoal = async (goalId, updatedData) => {
    ...
    try {
      ...
      const authHeader = await getAuthHeader();
      const response = await axios.put(`/api/finance/savings-goals/${goalId}`, updatedData, authHeader);
      ...
      setError('Error updating savings goal');
